...
if [[ -z "$BINARY_FILE" ]]; then
    echo -e "${RED}${SCRIPT_NAME} ${VERSION} error on line $LINENO: expected an argument!${NC}" >&2
    exit
elif [[ ! -f "$BINARY_FILE" ]]; then
    echo -e "${RED}${SCRIPT_NAME} ${VERSION} error on line $LINENO: specified file $BINARY_FILE is not exist!${NC}" >&2
    exit
elif [[ ! $BINARY_FILE == *.bin ]]; then
    echo -e "${RED}${SCRIPT_NAME} ${VERSION} error on line $LINENO: speficied file must have .bin extension!${NC}" >&2
    exit
elif [[ ! $FLASH_ADDRESS =~ ^0x[0-9a-fA-F]+$ ]]; then
    echo -e "${RED}${SCRIPT_NAME} ${VERSION} error on line $LINENO: flash address must be a hex value, got $FLASH_ADDRESS!${NC}" >&2
    exit
fi
...
for attempt in {1..25}; do
    ...
    if [[ -n "$is_programmer_not_found" ]]; then
